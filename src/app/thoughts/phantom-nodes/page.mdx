...
import { Appendix, Eq, M } from "@/components/Appendix";
...
<Sidenote label="Caveat" accent>
  ...
  venture-backed tech startups. They include restaurants, dry cleaners, and law
  firms alongside software companies. That makes them noisy. It may also make
  them more revealing. If the same stability holds across that much variety,
  the constraint is probably structural rather than methodological.
</Sidenote>
...
<Appendix title="Mathematical Appendix">

### Setup

Let <M math="G" /> be a directed acyclic graph (DAG) where each node represents a potential innovation, company, or opportunity. Each node <M math="i" /> has a set of parent nodes <M math="P(i)" /> — its prerequisites.

A node <M math="i" /> becomes **feasible** at time <M math="t" /> when all of its parents have been unlocked:

<Eq math="\text{feasible}(i, t) = 1 \iff \forall\, j \in P(i) : \text{unlocked}(j, t) = 1" />

A node that appears feasible to an observer but has unresolved prerequisites is a **phantom node**:

<Eq math="\text{phantom}(i, t) = 1 \quad \text{if} \quad \text{feasible}(i, t) = 0 \;\wedge\; \text{perceived\_feasible}(i, t) = 1" />
...
- <M math="F(t)" /> = number of feasible nodes (real opportunities)
- <M math="\Phi(t)" /> = number of phantom nodes (perceived but not yet feasible)
- <M math="N(t)" /> = number of new entrants (startups founded at time <M math="t" />)
- <M math="b" /> = average branching factor (child nodes exposed per unlock)

When a node is unlocked, it exposes on average <M math="b" /> new child nodes. Some of these will be immediately feasible (their other prerequisites already resolved), most won't. Let <M math="\alpha" /> be the fraction of newly exposed nodes that are immediately feasible. Then the frontier grows approximately as:

<Eq math="F(t+1) \;\approx\; F(t) + \alpha b \cdot \text{unlocks}(t) - \text{unlocks}(t)" />

The first term is new feasible nodes appearing. The second is feasible nodes being consumed by successful startups.

### The survival rate

A startup founded at time <M math="t" /> succeeds (survives to horizon <M math="\tau" />) if it is probing a real node and wins the competition at that node. The probability decomposes as:

<Eq math="S(\tau) \;\approx\; P(\text{real node}) \cdot P(\text{win} \mid \text{real node})" />

Where:

<Eq math="P(\text{real node}) = \frac{F(t)}{F(t) + \Phi(t)}" />
...
<Eq math="P(\text{win} \mid \text{real node}) \;\approx\; \frac{1}{k(t)}" />

Where <M math="k(t)" /> is the average number of entrants competing at each feasible node. This is where methods (search quality) and competitive dynamics (Red Queen) operate.

### Why the rate is stable

The key claim is that <M math="F(t) \,/\, [F(t) + \Phi(t)]" /> is approximately constant over time. This happens if the graph is self-similar: the ratio of feasible to phantom nodes at the frontier looks the same regardless of the frontier's size.
...
Additionally, <M math="N(t)" /> is coupled to <M math="F(t)" />. More feasible nodes → more visible opportunities → more entrants. If <M math="N(t)" /> scales roughly linearly with <M math="F(t)" />, then <M math="k(t)" /> stays roughly constant too. Both terms in the survival equation hold steady.

The result:

<Eq math="S(\tau) \;\approx\; \text{constant}" />
