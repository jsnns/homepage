...
The first step I took to find a generic definition was to make a big list of different problems in business, science, engineering, self-improvement, personal relationships, and economics, then read them a few times and let them sizzle in my unconscious while I went about life for a while. Eventually, halfway through watching 21 Jump Street with a couple of friends, a framework came to me and I'm taking a moment to write it down:

## Defining a generic problem

It turns out you can define a generic problem with just a few arguments. You have a current state `S` and a function `f` that describes its dynamics, plus a desired state `S'` that satisfies some condition or goal, all subject to a set of constraints `C`.
