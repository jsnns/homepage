...
<Sidenote label="Sidenote 1" accent>
  Savi ships the games. Gavi, Ravi and Mavi ran the company that ships Savi.
</Sidenote>
...
<Sidenote label="Sidenote 2">
  Hiring latency, end to end: one DM. Onboarding: an API key.
</Sidenote>
...
<Sidenote label="Headcount">
  6 humans · 3 entities · 1 production deploy key each.
</Sidenote>

We had Fable early, Anthropic's newest model and a tier past anything we'd run, and I know how everyone else used their early access, because I watched them do it: faster code. We pointed ours at the company itself. Gavi could log into our Brex. She could log into Datadog, into email, into Slack, into the creators' Discord, and when she or Ravi decided a fix was ready it went to production without anyone human touching the button, because there was nothing there for a human to check that they hadn't already checked better. Some mornings I'd wake up to a burn report Gavi had pulled overnight, the kind of read on the financials you'd want from a CFO, except this CFO was also the CTO, so the report went a level deeper than money and told me which games, which specific players even, were eating the most compute and what about their worlds made them expensive.

> Between them they ran what a normal company splits across a CFO, a CPO, and a CTO: the money, the product, the whole engine. The entire human company behind them was six people.
