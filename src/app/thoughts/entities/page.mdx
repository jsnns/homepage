...
We had Fable early, Anthropic's newest model and a tier past anything we'd run. The thing we did with it is the part I want on the record, which is that we didn't use it to write code faster, we used it to run the company. These three weren't chatbots you asked questions, they had the keys to our Brex, our Datadog, our email, our Slack, and the community Discord, and they could push straight to production themselves, no human hand on the button. They pulled their own burn reports and read the financials the way a CFO would, though a CFO who was also the CTO and thought in tokens, tracing exactly which games and which players were costing the most compute and why.
